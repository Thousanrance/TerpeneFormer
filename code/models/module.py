...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, key, value, query, mask=None, additional_mask=None,
        ...
        def shape(x):
            """  projection """ # (B,head,-1,dim_per_head)
            return x.view(batch_size, -1, head_count, dim_per_head) \
                .transpose(1, 2)

        def unshape(x):
            """  compute context """ # https://zhuanlan.zhihu.com/p/64551412 contiguous()使得内存连续加快读取速度
        ...
        if layer_cache is not None:
            if type == "self":
                ...
                # 推理过程交叉注意力key query value 投射分别来自于对应的key、query、value 
                ...
                if layer_cache is not None:
                    if layer_cache["memory_keys"] is None:
                        ...
                        # 不为空的话直接使用交叉注意力直接使用layer_cache中的内容
            ...
            # 正常训练过程key\value\query分别来自于对应的投射
            key_projected = self.linear_keys(key) # [batch, key_len, dim]
            value_projected = self.linear_values(value)
            query_projected = self.linear_query(query) # [batch, query_len, dim]
            key_shaped = shape(key_projected) # [batch, heads, key_len, dim_per_head]
            value_shaped = shape(value_projected)

        query_shaped = shape(query_projected) # [batch, heads, query_len, dim_per_head]
        ...
        if edge_feature is None and additional_mask is not None:
            # mask [batch_size x tgt_len x src_len] = [batch_size x query_len x key_len]
            # additional_mask [batch_size x key_len]
            query_shaped = query_shaped / math.sqrt(dim_per_head)
            # 对于注意力头一半global，一半local
            ...
            # Global:(B,head,query_len,dim_per_head) X (B,head,dim_per_head,key_len) = (B,head,query_len,key_len)
            score_global = torch.matmul(query_shaped_global, key_shaped_global.transpose(2, 3))
            # (B,1,query_len,key_len) top-score只取了一个头 
            top_score = score_global.view(batch_size, score_global.shape[1],
                                          query_len, key_len)[:, 0, :, :].contiguous()
            if mask is not None:
                #  [batch_size x query_len x key_len] -> [batch_size x heads x query_len x key_len] 
                mask = mask.unsqueeze(1).expand_as(score_global).clone()
                score_global = score_global.masked_fill(mask, -1e18) # 将mask的部分使用负无穷替代
            attn = self.softmax(score_global) # softmax 不改变形状
            drop_attn = self.dropout(attn)
            # [batch_size x global_heads x query_len x key_len] x [batch, global_heads, key_len, dim_per_head] ->[batch, global_heads, query_len, dim_per_head]
            global_context = torch.matmul(drop_attn, value_shaped_global)

            # Local:(B,head,dim_per_head,dim_per_head)
            #  [batch, local_heads, query_len, dim_per_head] x [batch, local_heads, dim_per_head, key_len] -> [batch, local_heads, query_len,key_len]
            score_local = torch.matmul(query_shaped_local, key_shaped_local.transpose(2, 3))
            if additional_mask is not None:
                # [batch_size x key_len]  -> [batch, local_heads, query_len,key_len]
            ...
            drop_attn = self.dropout(attn)
            # [batch, local_heads, query_len,key_len] x [batch, local_heads, key_len, dim_per_head] -> [batch, local_heads, query_len, dim_per_head]
            local_context = torch.matmul(drop_attn, value_shaped_local)

            # Merge:[batch, heads, query_len, dim_per_head]
            context = torch.cat([global_context, local_context], dim=1)
            context = unshape(context) # [batch, query_len, heads*dim_per_head]

        # Local-Global Encoder: 这是编码器
            ...
            # key_shaped_local [pair_indices[0], head_count // 2:, pair_indices[2]]
            # edge_feature_shaped [batch_size*valid_src_len*valid_src_len, head_count // 2, dim_per_head]
            key_shaped_local = key_shaped_local * edge_feature_shaped
            query_shaped_local = query_shaped_local / math.sqrt(dim_per_head)
            # ([pair_indices[0], head_count // 2:,1, pair_indices[1])  (pair_indices[0], head_count // 2:, pair_indices[2],1)
            ...
            attn_local = attn_local.masked_fill(score_expand_local < -10000, 0) # score_expand_local < -10000 的均使用0填充
