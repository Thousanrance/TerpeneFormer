...
class Embedding(nn.Module):
    def __init__(self, vocab_size, embed_size, dropout=0.1, padding_idx=1, USPTO_extend_vocab=False):
        super().__init__()
        self.vocab_size = vocab_size
        self.token = TokenEmbedding(vocab_size=vocab_size, embed_size=embed_size, padding_idx=padding_idx)
        if USPTO_extend_vocab:
            token_USPTO = TokenEmbedding(vocab_size=84+1, embed_size=embed_size, padding_idx=padding_idx)
            self.token.weight[:85,:].data.copy_(token_USPTO.weight[:,:]) 
