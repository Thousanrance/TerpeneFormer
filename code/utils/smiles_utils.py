...
from rdchiral.template_extractor import extract_from_reaction, get_changed_atoms, mols_from_smiles_list, \
    replace_deuterated
from attri.bond_length import BondLen_default
...
class SmilesGraph: # 不同于一般的分子图，这里作者提出了SmilesGraph的概念
    def __init__(self, smi, existing=None, bond_atti_dim=7, attri_bondlen=False):

        self.attri_bondlen = attri_bondlen
        self.bond_atti_dim = bond_atti_dim
        ...
        if self.bond_atti_dim ==7:
            ...
            self.full_adjacency_tensor = np.zeros((len(self.adjacency_matrix), len(self.adjacency_matrix), self.bond_atti_dim), dtype=float)
    ...
    def one_hot_vector(self, val, lst):
        ...
        if val not in lst:
            val = lst[-1]
        return map(lambda x: x == val, lst) # map 转换为1或0

    def get_bond_attributes(self, bond, bl=None):
        """
        ...
        if bl!=None:
            attributes.append(bl)

        return np.array(attributes)

    def get_default_BondLen(self, bond):
        BondLen=None
        ...
        b_type=bond.GetBondTypeAsDouble() # [1.0, 1.5, 2.0, 3.0]
        if b_type==1.0:
            BondLen = BondLen_default['SINGLE'][begin_symbol][end_symbol]
        elif b_type==1.5:
            BondLen = BondLen_default['AROMATIC'][begin_symbol][end_symbol]
        elif b_type==2.0:
            BondLen = BondLen_default['DOUBLE'][begin_symbol][end_symbol]
        elif b_type==3.0:
            BondLen = BondLen_default['TRIPLE'][begin_symbol][end_symbol]
        else:
            raise ValueError
        assert BondLen!=None
        return BondLen
    def extract_graph_structure(self, smi, verbose=False):
        ...
        a = SmilesGraph('CCCO')
        b = a.extract_graph_structure('CCCO')
        pprint(b)
        
        >>>:
                ([[1], [0, 2], [1, 3], [2]],
                {(0, 1): 3, (1, 0): 3, (1, 2): 3, (2, 1): 3, (2, 3): 3, (3, 2): 3},
                {(0, 1): array([ True, False, False, False, False, False, False]),
                (1, 0): array([ True, False, False, False, False, False, False]),
                (1, 2): array([ True, False, False, False, False, False, False]),
                (2, 1): array([ True, False, False, False, False, False, False]),
                (2, 3): array([ True, False, False, False, False, False, False]),
                (3, 2): array([ True, False, False, False, False, False, False])})
        """
        ...
        #         Chem.EmbedMolecule(mol3d, randomSeed=42, useRandomCoords=True)
        #         Chem.MMFFOptimizeMolecule(mol3d, maxIters=10000)
        #         conf = mol3d.GetConformer()
        # except:
        #     print(smi)

        sample_mol = Chem.RemoveHs(mol3d)
        ...
        for atom in sample_mol.GetAtoms():
            ...
            for i, neighbor_atom in enumerate(atom.GetNeighbors()):# 得到邻居的相关性质
                ...
                bl=None
                if self.bond_atti_dim==8 or self.attri_bondlen==True:
                    bl = self.get_default_BondLen(bond)
                bond_attri = self.get_bond_attributes(bond,bl)
                # print('bond attri', bond_attri.shape)
                if self.bond_atti_dim!=7:
                    bond_attri = bond_attri[-self.bond_atti_dim:]
                # print('new bond attri', bond_attri.shape)
                neighbor_bonds.append(str(bond.GetBondType()))
                neighbor_bonds_attributes.append(bond_attri)
            # 这是atom_symbols_i以及neighbor_tagged_smiles。知道了为什么使用Chem.MolFragmentToSmiles
            # ['[C:1]', '[C:900]', 'C', 'O']
            # [C:1][C:900]CO
            # ['[C:900]', '[C:1]', '[C:901]', 'O']
            # [C:900][C:1][C:901]O
        ...
        for ni, neighbor_tagged_smiles in enumerate(neighbor_smiles_list):
            ...
            if cur_i > -1:
                ...
                for cur_j in cand_js: # bond_types, bond_attributes均为字典，这里填充它们的键类型和键性质
...
def select_diverse_candidate(cc_trace_with_score, diverse_k=10): # 最大化多样性
...
def get_reaction_centers_from_template(src_smiles, blank_src_smiles, graph_pack, reaction_centers):
    ...
    return sorted(potential_rcs.items(), key=lambda x: -x[1]) # 它返回元组中第二个元素的负值。这表示对字典值进行降序排序，因为负值会反转元素的正常顺序。
...
def smi_tokenizer(smi):
    ...
    for s in ['<ENZ_10>', '<ENZ_1>', '<ENZ_2>', '<ENZ_3>', '<ENZ_4>', '<ENZ_5>', '<ENZ_6>', '<ENZ_7>', '<ENZ_8>', '<ENZ_9>', '<ENZ_unk>', 
              '<RX_10>', '<RX_1>', '<RX_2>', '<RX_3>', '<RX_4>', '<RX_5>', '<RX_6>', '<RX_7>', '<RX_8>', '<RX_9>', '<UNK>',
              '<rxnfp_10>', '<rxnfp_1>', '<rxnfp_2>', '<rxnfp_3>', '<rxnfp_4>', '<rxnfp_5>', '<rxnfp_6>', '<rxnfp_7>', '<rxnfp_8>', '<rxnfp_9>', '<rxnfp_unk>']:
        smi = smi.replace(s,'')
...
def SMI2IMG(df_path, img_folder,img_size):
    # 读取 DataFrame
    df = pd.read_csv(df_path)
    # df = df.iloc[1486:1498]
    # 创建 img_folder 文件夹
    ...
    for index, row in df.iterrows():
        ...
        pro_smi = row['pro_smi']
        # 根据 rxn_id 创建子文件夹
        rxn_folder = os.path.join(img_folder, str(rxn_id))
        if not os.path.exists(rxn_folder):
            os.makedirs(rxn_folder)
        ...
        for i, sub in enumerate(substrates):
            sub_smi_i = sub_smi.split('.')[i]  # 获取对应位置的 smiles 码
            mol = Chem.MolFromSmiles(sub_smi_i)
            ...
            if mol is not None:
                ...
                # 图片保存路径
                img_path = os.path.join(rxn_folder, f'sub_{sub}.png')

                # 保存图片
                img.save(img_path)
        mol = Chem.MolFromSmiles(pro_smi)
        if mol is not None:
            # 生成图片
            img = Draw.MolToImage(mol, size=(img_size, img_size))

            # 图片保存路径
            img_path = os.path.join(rxn_folder, f'pro_{product}.png')

            # 保存图片
            img.save(img_path)


def std_smi(smiles,basicClean=True,clearCharge=True, clearFrag=False, canonTautomer=False):
    try:
        ...
        def clear_mapnum(mol):
            ...
            return mol
        clean_mol = clear_mapnum(clean_mol)
        ...
        stan_smiles=Chem.MolToSmiles(clean_mol, isomericSmiles=True,canonical=True)
