...
def translate_batch_original(model, batch, eos_idx=3, beam_size=10, max_length=279,
                             invalid_token_indices=[], dataset=None, args=None):
    """
    ...
    src, tgt, rxn_class_label, gt_context_alignment, nonreactive_mask, graph_packs, images = \
        accumulate_batch(true_batch,use_multi_modal=args.use_multi_modal, args=args)
    bond, _ = graph_packs

    src, tgt, rxn_class_label, bond, gt_context_alignment, nonreactive_mask = \
            src.to(args.device), tgt.to(args.device), rxn_class_label.to(args.device), bond.to(args.device), \
            gt_context_alignment.to(args.device), nonreactive_mask.to(args.device)
    if images !=None:
    ...
    with torch.no_grad():
        ...
        prior_encoder_out, image_features, edge_feature = model.encoder(src, bond)

        atom_rc_scores = model.atom_rc_identifier(prior_encoder_out) # (bs,len,1)
        bond_rc_scores = model.bond_rc_identifier(edge_feature) if edge_feature is not None else None

        student_mask = model.infer_reaction_center_mask(bond, atom_rc_scores, bond_rc_scores)
        teacher_mask = nonreactive_mask

        rxn_class_scores = model.rxn_classifier(prior_encoder_out.mean(dim=0))  # (bs, rxn_num_class)
    ...
    predicted_rxn_class = rxn_class_scores.argmax(dim=-1).cpu().numpy()
    # (Sorting is done in explain_batch)
    return pred_tokens, pred_scores, predicted_rxn_class
